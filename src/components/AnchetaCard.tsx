...
export default function AnchetaCard({ ancheta }: AnchetaCardProps) {
  ...
  const handleWhatsAppClick = () => {
    const phoneNumber = "573104418272"; // Reemplaza con tu número de WhatsApp (código país + número)
  ...
  return (
    <div className="group bg-white/80 backdrop-blur-sm rounded-3xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-500 border border-rose-100 hover:border-rose-200 hover:-translate-y-1">
      {/* Imagen */}
      <div className="relative h-56 sm:h-64 bg-gradient-to-br from-amber-50 via-rose-50 to-pink-50 overflow-hidden">
      ...
      <div className="p-5 sm:p-6">
        <h3 className="font-display text-xl sm:text-2xl font-bold text-gray-800 mb-2 line-clamp-2">
          {ancheta.Nombre}
        </h3>

        <p className="text-sm sm:text-base text-gray-500 mb-4 line-clamp-2 leading-relaxed font-light">
          {ancheta.Descripcion}
        </p>
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <span className="font-display text-2xl sm:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-rose-400 to-pink-400">
              {ancheta.Precio}
            </span>
          </div>
        </div>
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-green-400 to-emerald-500 hover:from-green-500 hover:to-emerald-600 text-white font-semibold py-3 sm:py-3.5 px-6 rounded-2xl transition-all duration-300 transform hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2 shadow-md hover:shadow-lg"
        >
          <svg
